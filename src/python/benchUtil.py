...
def valid_path(logfilename: str):
    if osName in ("windows", "cygwin"):
        return logfilename[:2] + logfilename[2:].replace(":", "_")
    return logfilename
...
def nameToIndexPath(name):
    path = "%s/%s" % (constants.INDEX_DIR_BASE, name)
    if osName in ("windows", "cygwin"):
...
class SearchTask:
    ...
    def verifySame(self, other, verifyScores, verifyCounts):
        ...
        if self.groupField is None:
            if False:
                ...
                if self.expandedTermCount != other.expandedTermCount:
                    print(
                        "WARNING: expandedTermCounts differ for %s: %s vs %s"
                        % (self, self.expandedTermCount, other.expandedTermCount)
                    )
            ...
            if len(self.hits) != len(other.hits):
                self.fail(
                    "wrong top hit count: %s vs %s" % (len(self.hits), len(other.hits))
                )
            ...
            if verifyScores:
                ...
                if verifyCounts and len(hitsSelf) != len(hitsOther):
                    self.fail(
                        "self=%s: wrong collapsed hit count: %s vs %s\n  %s vs %s\n  %s vs %s"
                        % (
                            self,
                            len(hitsSelf),
                            len(hitsOther),
                ...
                if verifyScores:
                    for i in range(len(hitsSelf)):
                        if hitsSelf[i][1] != hitsOther[i][1]:
                            if False:
                                if (
                                    abs(float(hitsSelf[i][1]) - float(hitsOther[i][1]))
                                    > MAX_SCORE_DIFF
                                ):
                                    self.fail(
                                        "hit %s has wrong field/score value %s vs %s"
                                        % (i, hitsSelf[i][1], hitsOther[i][1])
                                    )
                                else:
                                    print(
                                        "WARNING: query=%s filter=%s sort=%s: slight score diff %s vs %s"
                                        % (
                                            self.query,
                                            self.filter,
                                            self.sort,
                                ...
                                self.fail(
                                    "hit %s has wrong field/score value %s vs %s"
                                    % (i, hitsSelf[i], hitsOther[i])
                                )
                        if hitsSelf[i][0] != hitsOther[i][0] and i < len(hitsSelf) - 1:
                            self.fail(
                                "hit %s has wrong id/s %s vs %s"
                                % (i, hitsSelf[i], hitsOther[i])
                            )
        else:
            # groups
            if self.groupCount != other.groupCount:
                self.fail(
                    "wrong groupCount: cat=%s groupField=%s %s vs %s: self=%s, other=%s"
                    % (
                        self.cat,
                        self.groupField,
                        self.groupCount,
            ...
            for groupIDX in range(self.groupCount):
                groupValue1, groupTotHits1, groupTopScore1, groups1 = self.groups[
                    groupIDX
                ]
                groupValue2, groupTotHits2, groupTopScore2, groups2 = other.groups[
                    groupIDX
                ]
                ...
                if False and groupValue1 != groupValue2:
                    self.fail(
                        "group %d has wrong groupValue: %s vs %s"
                        % (groupIDX, groupValue1, groupValue2)
                    )
                ...
                if verifyScores:
                    if groupTopScore1 != groupTopScore2:
                        self.fail(
                            "group %d has wrong groupTopScore: %s vs %s"
                            % (groupIDX, groupTopScore1, groupTopScore2)
                        )

                    if groupTotHits1 != groupTotHits2:
                        self.fail(
                            "group %d has wrong totHits: %s vs %s"
                            % (groupIDX, groupTotHits1, groupTotHits2)
                        )

                    if len(groups1) != len(groups2):
                        self.fail(
                            "group %d has wrong number of docs: %s vs %s"
                            % (groupIDX, len(groups1), len(groups2))
                        )
                    ...
                    for docIDX in range(len(groups1)):
                        if groups1[docIDX][1] != groups2[docIDX][1]:
                            self.fail(
                                "hit %s has wrong field/score value %s vs %s"
                                % (docIDX, groups1[docIDX][1], groups2[docIDX][1])
                            )
                        if (
                            groups1[docIDX][0] != groups2[docIDX][0]
                            and docIDX < len(groups1) - 1
                        ):
                            self.fail(
                                "hit %s has wrong id/s %s vs %s"
                                % (docIDX, groups1[docIDX][0], groups2[docIDX][0])
                            )
        ...
        if self.facets != other.facets:
            if False:
                print()
                print(
                    "***WARNING*** facet diffs: %s: %s vs %s"
                    % (self, self.facets, other.facets)
                )
                print()
            else:
    ...
    def fail(self, message):
        s = "query=%s filter=%s sort=%s groupField=%s hitCount=%s" % (
            self.query,
            self.filter,
            self.sort,
            self.groupField,
    ...
    def __hash__(self):
        return (
            hash(self.query)
            + hash(self.sort)
            + hash(self.groupField)
            + hash(self.filter)
            + hash(type(self.facets))
        )
...
reSearchTaskOld = re.compile(
    "cat=(.*?) q=(.*?) s=(.*?) group=null hits=(null|[0-9]+\+?) facets=(.*?)$"
)
reSearchGroupTaskOld = re.compile(
    "cat=(.*?) q=(.*?) s=(.*?) group=(.*?) groups=(.*?) hits=([0-9]+\+?) groupTotHits=([0-9]+)(?: totGroupCount=(.*?))? facets=(.*?)$",
    re.DOTALL,
)
reSearchTask = re.compile(
    "cat=(.*?) q=(.*?) s=(.*?) f=(.*?) group=null hits=(null|[0-9]+\+?)$"
)
reSearchGroupTask = re.compile(
    "cat=(.*?) q=(.*?) s=(.*?) f=(.*?) group=(.*?) groups=(.*?) hits=([0-9]+\+?) groupTotHits=([0-9]+)(?: totGroupCount=(.*?))?$",
    re.DOTALL,
)
...
reOneGroup = re.compile(
    "group=(.*?) totalHits=(.*?)(?: hits)? groupRelevance=(.*?)$", re.DOTALL
)
...
def parseResults(resultsFiles):
    ...
    for resultsFile in resultsFiles:
        ...
        if (
            os.path.exists(resultsFile + ".stdout")
            and os.path.getsize(resultsFile + ".stdout") > 50 * 1024
        ):
            raise RuntimeError(
                "%s.stdout is %d bytes; leftover System.out.println?"
                % (resultsFile, os.path.getsize(resultsFile + ".stdout"))
            )
            ...
            if line.startswith(b"TASK: cat="):
                ...
                if cat is not None:
                    ...
                    if sort in ('<string: "title">', '<string: "titleDV">'):
                        task.sort = "Title"
                    elif sort.startswith('<long: "datenum">') or sort.startswith(
                        '<long: "lastModNDV">'
                    ):
                        task.sort = "DateTime"
                    ...
                    if cat is not None:
                        ...
                        while True:
                            ...
                            if m is not None:
                                ...
                                # BUG
                                raise RuntimeError(
                                    "result parsing failed: line=%s" % line
                                )
    ...
    for tasksByCat in iters:
        ...
        for task, results in tasks[1].items():
            ...
            if SELECT == "min":
                ...
                raise RuntimeError(
                    "unrecognized SELECT=%s: should be min, median or mean" % SELECT
                )
        ...
        if lastHitCount is None:
            ...
            raise RuntimeError(
                "different hit counts: %s vs %s" % (lastHitCount, totHitCount)
            )
    ...
    if VERBOSE:
        ...
        print(
            ...
            % (
                ...
                cat[0],
                " ".join(
                    ["%5.1f" % (100.0 * (x - minValue) / minValue) for x in accumMS]
                ),
            )
        )
...
def stats(l):
    ...
    if len(l) == 0:
        ...
        return (
            ...
            max(l),
            sum / len(l),
            math.sqrt(len(l) * sumSQ - sum * sum) / len(l),
        )
...
def run(cmd, logFile=None, indent="    "):
    ...
    if p.wait():
        ...
        raise RuntimeError(
            "failed: %s [wd %s]; see logFile %s" % (cmd, os.getcwd(), logFile)
        )
...
class RunAlgs:
    ...
    def makeIndex(
        self, id, index, printCharts=False, profilerCount=30, profilerStackSize=1
    ):
        ...
        try:
            ...
            jfrOutput = valid_path(jfrOutput)
            ...
            fullLogFile = valid_path(fullLogFile)
            ...
            with open(fullLogFile, "rb") as f:
                while True:
                    ...
                    if l.lower().find(b"exception in thread") != -1:
                        raise RuntimeError(
                            'unhandled exceptions in log "%s"' % fullLogFile
                        )
        ...
        try:
            if competitor.checkout not in self.compiledCheckouts:
                ...
                for module in ["core"]:
                    ...
                    run(
                        [constants.GRADLE_EXE, "lucene:core:jar"],
                        "%s/compile.log" % constants.LOGS_DIR,
                    )
                for module in (
                    "suggest",
                    "highlighter",
                    ...
                    if (
                        common.getLatestModTime("%s/src/java" % modulePath)
                        > lastCompileTime
                    ):
                        print("compile lucene:%s..." % module)
                        run(
                            [constants.GRADLE_EXE, "lucene:%s:compileJava" % module],
                            "%s/compile.log" % constants.LOGS_DIR,
                        )
        ...
        try:
            if competitor.checkout not in self.compiledCheckouts:
                ...
                for module in (
                    ...
                    modulePath = "%s/lucene/%s" % (checkoutPath, module)
                    classesPath = "%s/lucene/build/%s/classes/java" % (
                        checkoutPath,
                        module,
                    )
                    ...
                    if common.getLatestModTime(
                        "%s/src/java" % modulePath
                    ) > common.getLatestModTime(classesPath, ".class"):
                        print("  %s..." % modulePath)
                        os.chdir(modulePath)
                        run(
                            [constants.GRADLE_EXE, "compileJava"],
                            "%s/compile.log" % constants.LOGS_DIR,
                        )
    ...
    def runSimpleSearchBench(
        self, iter, id, c, coldRun, seed, staticSeed, filter=None, taskPatterns=None
    ):
        ...
        if coldRun:
            ...
            if osName == "linux":
                ...
                raise RuntimeError(
                    "do not know how to purge buffer cache on this OS (%s)" % osName
                )
    ...
    def computeTaskLatencies(self, inputList, catSet):
        ...
        for currentRecord in inputList:
            for currentKey in currentRecord.keys():
                ...
                currentP90 = currentCatLatencies[
                    int((len(currentCatLatencies) - 1) * 0.9)
                ]
                currentP99 = currentCatLatencies[
                    int((len(currentCatLatencies) - 1) * 0.99)
                ]
                currentP999 = currentCatLatencies[
                    int((len(currentCatLatencies) - 1) * 0.999)
                ]
    ...
    def simpleReport(
        ...
        cmpDiffs = compareHits(
            baseRawResults, cmpRawResults, self.verifyScores, self.verifyCounts
        )
        ...
        for cat in cats:
            ...
            if (qpsStdDevBase != 0 or qpsStdDevCmp != 0) and len(baseQPS) == len(
                cmpQPS
            ):
                ...
                qpsStdDev = math.sqrt(
                    (qpsStdDevBase * qpsStdDevBase + qpsStdDevCmp * qpsStdDevCmp) / 2.0
                )

                # t-value is the difference of the means normalized by the combined std.dev.
                if qpsStdDev != 0 and len(baseQPS) != 0:
                    tValue = abs(qpsCmp - qpsBase) / (
                        qpsStdDev * math.sqrt(2.0 / len(baseQPS))
                    )
                else:
                    tValue = float("inf")
            ...
            if self.verifyCounts and baseTotHitCount != cmpTotHitCount:
                warnings.append(
                    "cat=%s: hit counts differ: %s vs %s"
                    % (desc, baseTotHitCount, cmpTotHitCount)
                )
            ...
            if jira:
                ...
                w(
                    "<td>%.1f</td><td>%s</td><td>%.1f</td><td>%s</td>"
        ...
        for currentCat in catSet:
            ...
            pctP50 = (
                100
                * (currentCmpMetrics["p50"] - currentBaseMetrics["p50"])
                / currentBaseMetrics["p50"]
            )
            pctP90 = (
                100
                * (currentCmpMetrics["p90"] - currentBaseMetrics["p90"])
                / currentBaseMetrics["p90"]
            )
            pctP99 = (
                100
                * (currentCmpMetrics["p99"] - currentBaseMetrics["p99"])
                / currentBaseMetrics["p99"]
            )
            pctP999 = (
                100
                * (currentCmpMetrics["p999"] - currentBaseMetrics["p999"])
                / currentBaseMetrics["p999"]
            )
            pctP100 = (
                100
                * (currentCmpMetrics["p100"] - currentBaseMetrics["p100"])
        ...
        if jira:
            w(
                "||Task||QPS %s||StdDev %s||QPS %s||StdDev %s||Pct diff||p-value||"
    ...
    def compare(self, baseline, newList, *params):
        ...
        for new in newList:
            if new.numHits != baseline.numHits:
                raise RuntimeError(
                    "baseline found %d hits but new found %d hits"
                    % (baseline[0], new[0])
                )
...
def getAntClassPath(checkout):
    ...
    if core_jar_file is None:
        raise RuntimeError(
            "can't find core JAR file in %s" % ("%s/lucene/build/core" % path)
        )
...
def getClassPath(checkout):
    ...
    if core_jar_file is None:
        raise RuntimeError(
            "can't find core JAR file in %s" % ("%s/lucene/core/build/libs" % path)
        )
    ...
    for root_path, dirs, files in os.walk(
        os.path.expanduser("~/.gradle/caches/modules-2/files-2.1/com.carrotsearch/hppc")
    ):
        for file in files:
    ...
    if not found:
        raise RuntimeError(
            f"unable to locate {hppc_dependency} dependency for lucene/facet!"
        )
        ...
        for tasks in taskIters[1:]:
            for task in tasks:
                if task not in d:
                    # BUG
                    raise RuntimeError(
                        "tasks differ from one iteration to the next: task=%s"
                        % str(task)
                    )
                else:
...
def compareHits(r1, r2, verifyScores, verifyCounts):
    ...
    if len(d1) != len(d2):
        ...
        warnings.append(
            "non-overlapping tasks onlyInD1=%s onlyInD2=%s" % (onlyInD1, onlyInD2)
        )
